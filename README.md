...
## Overview
This repository contains a high-performance C++ implementation of the **Pigeonhole Sorting Algorithm**. Unlike standard comparison-based sorts (e.g., Quicksort, Mergesort) which are bounded by $O(n \log n)$ time complexity, this implementation utilizes a non-comparison counting strategy to achieve $O(n + k)$ linear time complexity.
...
* **Limitations:** The primary bottleneck is the **Memory Requirement**. If the input range ($k$) significantly exceeds the number of items ($n$), or if the range is massive (e.g., $k > 10^9$), the memory consumption becomes inefficient or exceeds system capabilities. In such cases, comparison-based algorithms like Introsort are preferred.
